namespace :dev do
  ...
  desc 'Resetando o contador do subject'
  task reset_subject_counter: :environment do
    show_spinner('Resetando contador dos assuntos...') do
      Subject.all.each do |subject|
        Subject.reset_counters(subject.id, :questions_count)
      end
    end
  end

  desc 'Adicionando todas as respostas no Redis'
  task add_answers_to_redis: :environment do
    show_spinner('Adicionado todas as respostas no Redis...') do 
  ...
  def select_true_answer(answers_array)
    ...
    answers_array[select_index][:correct] = true
  end
